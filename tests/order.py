...
class OrderTests(APITestCase):
    def setUp(self) -> None:
        ...
        url = "/paymenttypes"
        data = {"merchant_name": "Chase", "account_number": "1234-5678-9012", 
                "customer": 1, "expiration_date": "2026-12-12", "create_date": "2026-05-16"}
        self.client.credentials(HTTP_AUTHORIZATION='Token ' + self.token)
        response = self.client.post(url, data, format="json")
        json_response = json.loads(response.content)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_complete_order(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(int(json_response["payment_type"].split("/")[-1]), data["payment_type"])
    ...
    # TODO: Get ALL Orders for a Customer